...
public class SessionHandler extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        /* Session
        * - 클라이언트가 최초로 서버에 요청을 보넬때 생성되는 객체
        * - 서버에서 해당 객체를 관리함
        * - 생성된 Session 객체의 ID를 클라이언트에게 전달(쿠키 이용)
        * - 이후 요청 시 클라이언트의 쿠키(Session ID)를 담아와서
        *       서버에 Session ID가 일치하는 Session 객체가 있는지 확인
        *  -> 있으면 해당 객체를 반환해서 활용
        *       -> 이 때, 반환 받은 객체에 값이 세팅되어 있다면 활용가능 == 상태 유지
        *
        ...
        * */
        ...
        // session 기본 유지 시간 지정 (초단위, 기본값 = 미설정시 30분)
        session.setMaxInactiveInterval(60 * 10); // 최대값 지정 (어제까지 할것인가)

        String firstName = request.getParameter("firstName");
        String lastName = request.getParameter("lastName");

        /* 세션 객체에 값을 속성으로 추가
        *   -> 같은 클라이언트가 다음에 요청을 하는경우
